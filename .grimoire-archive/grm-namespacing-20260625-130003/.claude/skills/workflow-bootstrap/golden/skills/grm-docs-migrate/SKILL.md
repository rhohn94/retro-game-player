---
name: docs-migrate
description: Detect and migrate old-style docs to the wiki hierarchy — detect mode classifies FLAT_TIER/ORPHAN/ABSOLUTE_LINK/PROSE_LINK/NO_BREADCRUMB findings; --apply archives then rewrites (breadcrumbs, relative links). Downstream-safe. Triggers on "migrate docs", "fix breadcrumbs", "convert absolute links", "run docs migration", "detect doc hierarchy findings".
---
...
- After `sync-from-upstream` detects old-style docs and offers migration.
- When `install-doctor` reports `DOCS_LEGACY_STYLE` findings.
...
```bash
python3 .claude/skills/docs-migrate/docs_migrate.py
```
...
```bash
python3 .claude/skills/docs-migrate/docs_migrate.py --apply
```
...
```bash
python3 .claude/skills/docs-migrate/docs_migrate.py --self-test
```
...
```bash
python3 .claude/skills/docs-migrate/docs_migrate.py --dry-run
python3 .claude/skills/docs-migrate/docs_migrate.py --apply --dry-run
```
...
```bash
python3 .claude/skills/docs-migrate/docs_migrate.py --docs-root path/to/docs
```
...
After a clean `--apply` (Step 3), `sync-from-upstream` checks for old-style
