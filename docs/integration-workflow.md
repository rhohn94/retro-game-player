...
- The integration master invokes `feedback-to-issue` directly for a single item.
...
roadmap when `issue-tracker` is configured in `grimoire-config.json`.

### Pre-digested context brief (cache-hit lever)

Every spawned session starts cold and pays a fixed cold preamble (~6 K tokens,
see `docs/token-efficiency-baseline.md`) plus the cost of re-reading the same
...
- **Who synthesizes:** the integration master, in the `release-phase` session,
...
`release-phase` Step 5 builds this digest and slots it into the spawn prompt's
context block.
...
receive feedback and file it through the `feedback-to-issue` skill. It is an
**optional additional channel**: the integration master may file one item via
`feedback-to-issue` directly; spawn a Reporter when filing multiple items, when
you want to keep the integration session uncontaminated, or when filing can run
in parallel with other work. Full role definition: `.claude/skills/reporter/SKILL.md`.
...
> [`docs/design/agent-roles-design.md`](design/agent-roles-design.md). The table
...
declines — see the `project-release` skill §GitHub Release.
