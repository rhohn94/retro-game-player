...
triggers, on every agent that uses it. The v1.29 `doc-assurance` **skill-budget**
...
- Tie the convention to the v1.9 methodology (it *is* Lever 4 applied to the
  worthwhile class) and to the existing `token-measure` protocol for per-op A/B.
...
- A new config dial or a hard gate that blocks a release on skill size (the
  budget stays a `doc-assurance` warn-level finding).
...
### Worked example (this release): `cost-budget`

`cost-budget` was reference-heavy (16,890 bytes): a config schema + procedure
...
`doc-assurance` budget metric and a sound proxy for the token footprint; for a
per-operation token A/B, use the v1.9 `token-measure` protocol on the skill's
session transcript.)
...
`onboarding` (46.5 KB), `design-language-adapt` (27.8 KB), `workflow-scaffold`
(25.6 KB), `workflow-bootstrap` (24.7 KB), `integration-master` (19.3 KB),
`release-phase-merge` (15.9 KB), `ux-demo-regress` (15.1 KB), `sync-from-upstream`
(14.7 KB), `researcher` (13.7 KB), `triager` (13.4 KB), `hard-reset` (13.1 KB),
`scout` (12.1 KB). The `doc-assurance` skill-budget check is the live tracker —
each future split removes one finding.

## Validation / Idempotency

- `wc -c .claude/skills/cost-budget/SKILL.md` → 8,991 B (was 16,890; under the
  12,000 budget). `reference.md` exists and carries §3–§8 verbatim.
- `doc-assurance` skill-budget findings: **13 → 12** (cost-budget removed); no
...
- Flavor parity: `cost-budget` head + reference mirrored to `claude-code/`.

## Flavor parity

`cost-budget` is a Claude-Code skill; its `SKILL.md` + `reference.md` are
mirrored to `claude-code/.claude/skills/cost-budget/`. This design doc is
...
The convention is now **automated**: `.claude/skills/doc-assurance/split_skill.py`
...
with a Bash call. `.claude/skills/token-measure/footprint.py` reports the static
baseline. Over-budget `doc-assurance` findings name `split_skill.py` as the fix.
