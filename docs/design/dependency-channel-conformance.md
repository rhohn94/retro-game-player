...
> `sync-deps` / `vendor-migrate`). The mechanism is a **git submodule**, not a
...
The Dependency Channel (`sync-deps`) reconciles first-party deps from **published
...
that is [ux/design-language.md](ux/design-language.md). The `sync-deps` /
`vendor-migrate` engine internals — those are the skill docs under
`.claude/skills/`.
...
The `sync-deps` engine's release-channel model **cannot express a submodule**,
...
gate**. This is precisely the situation `vendor-migrate` handles with its
...
- An active **`[submodules.aura]`** table recording `repo`, `url`, `channel`,
  ...
  read by the `sync-deps` engine (which consumes only `[deps.*]`); it is
...
1. **Engine gate (release-asset deps):** `sync-deps --check` and
   ...
   ```bash
   python3 .claude/skills/sync-deps/sync_deps.py --check     # exit 0: no drift
   python3 .claude/skills/sync-deps/sync_deps.py --offline   # exit 0: validates, zero network
   ```
...
- Dependency Channel consumer engine: `.claude/skills/sync-deps/SKILL.md`
- Submodule → channel migration helper: `.claude/skills/vendor-migrate/SKILL.md`
