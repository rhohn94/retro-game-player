...
| [native-emulation-design.md](native-emulation-design.md) | v0.21: native libretro core hosting (NES `fceumm` first, behind a flag) to fix the EmulatorJS cold-start audio garble (#15) and WASM load time at the source — hand-rolled `libloading` FFI (no viable hosting crate exists), `cpal`/CoreAudio audio with ring-buffer + dynamic rate control, canvas/IPC frame delivery (NSView overlay deferred), EmulatorJS kept as the fallback for every other system |
...
| [direct-download-design.md](direct-download-design.md) | v0.24 (#30) **implemented**: user-clicked download → staging → import → play; multi-hop auto-import; trust-tier provider priorities |
| [auto-import-download-design.md](auto-import-download-design.md) | HTML detail page → file hop, magic/HEAD, multi-hop, host profiles |
| [global-catalog-design.md](global-catalog-design.md) | Library Personal \| Global catalog browse (bundled titles → Find downloads) |
| [search-result-quality-design.md](search-result-quality-design.md) | SERP quality, query compose, health memory, console aliases |
| [web-meta-search-design.md](web-meta-search-design.md) | DuckDuckGo meta-search (not Yandex) |
| [rom-provider-seeds-design.md](rom-provider-seeds-design.md) | Research ROM seeds, priority bands, discovery UX |
