...
| `docs/integration-workflow.md` | Current flow: user-confirms at push, release, major merges. | Current flow + explicit user-led design sections; decision log. | Autonomous flow: master drives phases; push remains human-gated (v1.6 non-goal). |
...
3. For each entry in the install map (§2.2):
   ...
   c. For `docs/integration-workflow.md`: overwrite with source.
