...
plus `grm-sync-deps`/`vendor-check` delegating to framework scripts), the canonical
...
| `grm-sync-deps` | `sync-deps mode=""` | universal (delegates) | Reconcile / vendor first-party deps — delegates to the framework `grm-sync-deps` script. |
...
The `grm-sync-deps` / `vendor-check` recipes are **not** stubs — real, thin recipes
...
`grm-sync-deps`/`vendor-check` it never needs a project rewrite, so web/service ship
...
| `mode` | string | `""` | `grm-sync-deps` mode flag (`--check` / `--update` / `--offline`). |
