-- 013_gog_itch_sources.sql (v0.32 W320 — GOG + itch game sources)
--
-- Extends the `games.source` CHECK constraint (introduced by
-- 012_romless_games.sql) to accept two more non-ROM sources: 'gog' (GOG
-- Galaxy installs) and 'itch' (itch app installs). SQLite cannot `ALTER`
-- an existing CHECK constraint, so this migration rebuilds `games` again via
-- the same documented 12-step pattern as 012_romless_games.sql — same
-- foreign_keys-off requirement (see `Migration::requires_fk_off` /
-- `db/migrations.rs`), same reasoning: `DROP TABLE games` would otherwise
-- cascade-delete every `art_cache` row referencing it.
--
-- No data migration is needed beyond the straight copy: no existing row can
-- have source = 'gog' | 'itch' yet (those values didn't exist before this
...
CREATE TABLE games_new (
  ...
  source             TEXT    NOT NULL DEFAULT 'rom'
                       CHECK (source IN ('rom', 'steam', 'app', 'manual', 'gog', 'itch')),
