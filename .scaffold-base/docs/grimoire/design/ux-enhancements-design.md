...
> machinery — the per-project authority [`ux/design-language.md`](../../design/ux/design-language.md),
...
A new **`docs/design/ux/`** sub-layout, with [](../../design/ux/design-language.md) remaining the
...
[](../../design/ux/design-language.md)): the tiers are independently consumed (a token-only
tool needs [](../../design/ux/theme.md) without parsing component prose), independently
...
(or will have) more than one doc." [](../../design/ux/design-language.md) stays the orientation
anchor and cross-links the two.

#### Schema — [](../../design/ux/theme.md)
...
#### Schema — [](../../design/ux/components.md)
...
- **No raw values in [](../../design/ux/components.md)** — every visual property resolves
  through a `theme.*` reference (or a documented transform like
  `darken(token, n%)`). This is the layer's core invariant: re-theme by
  editing [](../../design/ux/theme.md) only; component recipes are stable across themes.
...
[](../../design/ux/theme.md) and [](../../design/ux/components.md) as **drafts**:

1. Map upstream Aura token scales → [](../../design/ux/theme.md) structured block, in the
   project's `token-syntax` (seeded by §1 detection).
2. Map the upstream control taxonomy → [](../../design/ux/components.md) recipes, each
...
3. Both files inherit the same lifecycle as [](../../design/ux/design-language.md): written as
...
- [](../../design/ux/design-language.md) stays the **authority and narrative**; it gains a
  `### Theme & components` subsection that links to [](../../design/ux/theme.md)/[](../../design/ux/components.md)
  and states which tiers the project populated.
- The **Adaptation-acceptance checklist** in [](../../design/ux/design-language.md) is unchanged
...
- `grm-ux-demo-build` reads [](../../design/ux/components.md) to know **which controls to build** and
  [](../../design/ux/theme.md) for the **values to apply** — replacing today's prose-parse of
...
- **Backward-compatible:** a project that never adds the tier files keeps a
  valid single-file [](../../design/ux/design-language.md). The tiers are additive; their absence
  is not an error.
...
- `visual-regression.json` is the manifest mapping each acceptance-checklist
  ...
  device-pixel-ratio, the [](../../design/ux/theme.md)/[](../../design/ux/components.md) SHA the baseline was taken
...
whether the baseline's recorded [](../../design/ux/theme.md)/[](../../design/ux/components.md) SHA differs from the
...
- [ ] #5: the design defines a two-tier `theme` (tokens) + `components`
      ...
      layout under `docs/design/ux/` ([](../../design/ux/theme.md) + [](../../design/ux/components.md) alongside
      the unchanged [](../../design/ux/design-language.md)), and states the no-raw-values-in-
...
- Token-syntax codegen (emitting actual CSS custom properties / `colors.xml`
  from [](../../design/ux/theme.md)) — designed as a `token-syntax` hint here but the generator
  itself is unscoped for v1.18.

## Follow-ups

- Populate concrete per-project [](../../design/ux/theme.md) / [](../../design/ux/components.md) sets for the
  dogfood project and any GUI reference project (downstream of v1.18).
- Optional token-codegen step turning [](../../design/ux/theme.md) into stack-native token
  artifacts.
