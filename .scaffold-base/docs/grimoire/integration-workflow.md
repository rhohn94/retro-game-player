...
### Flagging out-of-scope issues from spawned sessions

When a spawned work-item session discovers something out of scope (scope creep,
a follow-up bug, a deferred item worth tracking), it should **not** append
bullets directly to `docs/roadmap.md ## Backlog`. Instead the integration master
routes the flag through the issue-tracker abstraction:

- The integration master invokes `grm-feedback-to-issue` directly for a single item.
- The integration master spawns a Reporter (see §Filing issues with the Reporter
  below) for multiple items or when session isolation is preferred.
...
roadmap when `grm-issue-tracker` is configured in `grimoire-config.json`.

### Pre-digested context brief (cache-hit lever)

Every spawned session starts cold and pays a fixed cold preamble (~6 K tokens,
see `docs/grimoire/token-efficiency-baseline.md`) plus the cost of re-reading the same
shared background files that *every* agent in the batch would open — the design
doc overview, the standing rules, the conflict map. That shared read is paid
once per agent, multiplied by fan-out width.

To cut it, **the integration master synthesizes a single ≤800-token digest of
the shared background once** and embeds it verbatim in each spawn prompt. Each
agent reads the digest instead of re-deriving the same context cold, so the
shared reading is paid once (by the master) instead of N times.

- **Who synthesizes:** the integration master, in the `grm-release-phase` session,
  once per batch — *before* the first `spawn_task` call. Not the spawned agents.
- **Size cap:** **≤ 800 tokens.** It is a digest, not a dump. If the shared
  context cannot be summarized under the cap, link the file by path and let the
  agent read the specific part it needs — do not inline the whole file.
- **What it contains** (only the bits common to the whole batch):
  - the release's one-line goal + the current phase's theme;
  - the feature/design overview the items share (key decisions, not full text);
  - the §3 conflict map rows relevant to this batch (who-touches-what);
  - standing constraints already known (test/build commands, the worktree-root
    rule, "do not touch the release-planning doc");
  - pointers (by path) to the authoritative docs for anything that exceeds the
    cap — the agent still owns reading its *own* item's §2.{N} scope in full.
- **What it is NOT:** it does **not** replace per-item scope (each agent still
  reads its own §2.{N}), and it does **not** relax worktree isolation — every
  agent still gets its own fresh worktree and roots on `version/{X.Y}`. It is
  purely a cache-hit / cold-read-avoidance lever, not a scope or isolation
  change.

The shared digest is identical across the batch, so embedding it in each prompt
maximizes cache reuse; the per-item scope is the only part that varies.

`grm-release-phase` Step 5 builds this digest and slots it into the spawn prompt's
context block.
...
## Filing issues with the Reporter

The **Reporter** is a third named agent role — alongside the task agent and the
integration master — available in all three paradigms. Its sole job is to
receive feedback and file it through the `grm-feedback-to-issue` skill. It is an
**optional additional channel**: the integration master may file one item via
`grm-feedback-to-issue` directly; spawn a Reporter when filing multiple items, when
you want to keep the integration session uncontaminated, or when filing can run
in parallel with other work. Full role definition: `.claude/skills/grm-reporter/SKILL.md`.

### Agent-type taxonomy

> **Canonical registry:** the authoritative agent-role taxonomy — every role's
> session mandate, context width, git/issue write surface, spawn + return
> contract, model/effort pin, and per-paradigm behaviour — lives in
> [`docs/grimoire/design/agent-roles-design.md`](grimoire/design/agent-roles-design.md). The table
> below is the integration-master quick reference for the three roles relevant
> here; consult the registry for the full roster (including the Reviewer, Scout,
> Verifier, Triager, and Researcher roles) and to add or change a role.
...
The Reporter is **not** a paradigm role and has no associated worktree or branch.
It is a one-shot session: file all items, return issue number(s) and URL(s), exit.
...
run concurrently with an in-flight integration session or a phase merge. If the
configured tracker is `roadmap`, the Reporter appends to `docs/roadmap.md`
`## Backlog` on `dev` only — it stops and reports a conflict rather than
appending on a `version/*` or `main` branch.

If more feedback arrives after the Reporter exits, spawn a new Reporter — do not
reuse or idle a running one.

### Paradigm behaviour

- **Supervised:** each Reporter spawn requires explicit user confirmation via the
  standard `spawn_task` gate. The integration master proposes the spawn; the user
  approves before the session starts.
- **Weiss (Collaborative):** the integration master offers to spawn a Reporter
  and waits for user confirmation. It does not auto-spawn.
- **Noir (Autonomous):** the integration master spawns Reporters autonomously
  during planning, review, or merge phases — no per-spawn confirmation. Reporters
  may be batch-spawned at the end of a phase merge for all flagged items. The
  Reporter's narrow context keeps per-spawn cost low (~Haiku / Eco tier).

Across all paradigms: **the Reporter never pushes to origin** — that remains
human-gated.
...
**Optional GitHub Release.** At this same single post-release moment, the
integration master may optionally publish a GitHub Release from the version tag
(`gh release create`, notes drawn from `version-history.md`). It is human-gated
exactly like the push and degrades gracefully when `gh` is absent or the user
declines — see the `grm-project-release` skill §GitHub Release.
