...
the new form (e.g. `skills/grm-architecture-audit/` beside
...
`grm-sync-deps`/`vendor-check`; INTERFACE `server` surfaces as `run`), reporting per
recipe:
...
`grm-sync-deps`/`vendor-check`; INTERFACE `server` surfaces as `run`). Each is
