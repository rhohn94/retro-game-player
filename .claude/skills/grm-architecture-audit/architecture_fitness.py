...
"no rules declared" and the process exits 0 (never fails a project that has not
opted in).
...
def run_audit(root: str, rules_path: Optional[str] = None, gate: bool = False,
    ...
    if rules is None:
        msg = "architecture-audit: no rules declared"
        if as_json:
            print(json.dumps({"rules_declared": False, "findings": []}, indent=2))
        else:
            print(msg)
        return 0
...
def self_test() -> int:
    ...
    # 1. Absent rules file -> "no rules declared", exit 0.
